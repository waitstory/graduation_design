...
#define ThreL 20  //温度下限阈值
#define ThreH 25  //温度上限阈值
    ...
    while(1)
    {
        if(flag == 1)
        {
            ...
            if((RxStr[0] == 'S') && (RxStr[1] == 'Y') && (RxStr[2] == 'Y') && (RxStr[7] == 'H'))
            {
                ...
                if((RxStr[3]!='-')&&(RxStr[4]!='-')&&(RxStr[5]!='-')&&(RxStr[6]!='-'))    //当传感器在线的时候才计算数值    
                {
                    TemptSum = (RxStr[3]-'0')*10+(RxStr[4]-'0');  //温度值
                }
                if((TemptSum >= ThreL) && (TemptSum <= ThreH))      //判断接收的温度数据是否在阈值范围，上限(25)和下限(20)
                {
                    TemptSum = 0;
                    BEEP = 0;
                    LED2 = 0;
                ...
                if(TemptSum > ThreH)   //判断接收的温度数据是否超过阈值上限(25)
                {
                    TemptSum = 0;
                    BEEP = 0; 
                    LED1 = 0;
        ...
        else
        {
            ...
            {
                i=0;
                for(j=0;j<5;j++)  //判断是否四次接收的数据都为1，即从机没有响应
                {
                    if(temp[j] != 1)
                    {
                        break;
                    ...
                    else
                    {
                        ...
                        LcdShowChr(4,1,'0');    //显示温度的个位
                        LcdShowChr(11,1,' ');   //显示湿度的十位
                        LcdShowChr(12,1,'0');   //显示湿度的个位
                    }
                }
            }    
        }
